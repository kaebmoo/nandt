...
schema_name = f"tenant_{subdomain}"          # หรือ resolve จาก public.hospitals.schema_name (กัน hyphen)
...
- `schema_name` มาในรูปแบบ `tenant_<subdomain>` เช่น `tenant_humnoi`
...
def classify_on_checkin(appointment, now: datetime, policy: GracePolicy) -> str:
    """
    คืนค่า entry_class:
    - ถ้า now อยู่ในช่วง [slot_start - grace_before, slot_end + grace_after] -> 'appointment' (priority เต็ม)
...
def generate_sessions(service_point_id: int, date_from: date, date_to: date) -> list["Session"]:
    ...
    resolve availability ที่ effective ของแต่ละวันในช่วง [date_from, date_to]:
      ...
      3. apply date_overrides (template-specific) ทับ -> เพิ่ม/ลด/ปิดวันนั้น
    UPSERT เข้า sessions (UNIQUE service_point_id, session_date, name) -> idempotent
    ห้ามลบ/แก้ session ที่มี queue_entries อยู่แล้ว (รักษา history)
    """
...
| 1B.3 | trigger re-sync ตอน save availability/override | `availability_routes.py` | แก้ availability → session อนาคต update, อดีตไม่แตะ |
